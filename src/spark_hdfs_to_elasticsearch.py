...
def send_to_es_index(index_name: str, id_cols: list[str], partition):
    batch = list(partition)
    if not batch:
        return
...
# Mức 2-3 - Trend, MA, Growth, CFR 7d
...
# Mức 4 - Rankings (TopN + top in region)
...
# Mức 6 - Segmentation + peak + pre/post
...
quality_out = (
    quality_global.crossJoin(neg_quality)
)

# Ghi STATS theo nhóm index
print("\nĐang ghi STATS theo nhóm index vào Elasticsearch...")

country_sender = partial(send_to_es_index, ES_COUNTRY_STATS_INDEX, ["Country"])
region_sender = partial(send_to_es_index, ES_REGION_STATS_INDEX, ["WHO_region"])
global_daily_sender = partial(send_to_es_index, ES_GLOBAL_DAILY_INDEX, ["Date_reported"])
country_daily_sender = partial(send_to_es_index, ES_COUNTRY_DAILY_INDEX, ["Country", "Date_reported"])
anomalies_sender = partial(send_to_es_index, ES_ANOMALIES_INDEX, ["Country", "Date_reported", "type"])
seg_sender = partial(send_to_es_index, ES_SEGMENTATION_INDEX, ["Country"])
quality_sender = partial(send_to_es_index, ES_QUALITY_INDEX, ["scope", "key"])
rank_sender = partial(send_to_es_index, ES_RANKINGS_INDEX, ["metric", "WHO_region", "rank"])
